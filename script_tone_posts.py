...
async def process_item(item, session, sem, api_base, model, prompt_prefix, max_tokens, out_dir, groups_result, pbar):
    ...
    tone_post = (await gen(session, sem, api_base, model, post_prompt, max_tokens=max_tokens,
                           stats_path="run_stats_tone_posts.jsonl",
                           meta={"task": "tone_post", "group_id": group_id, "post_id": post_id})).strip()
    post_result = {"post_id": post_id, "post_link": post_link, "text": post_text, "tone_post": tone_post}
...
async def async_main():
    ...
    pbar = tqdm(total=len(data), desc="tone_posts")
